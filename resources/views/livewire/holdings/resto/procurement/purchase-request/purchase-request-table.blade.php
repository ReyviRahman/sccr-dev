...
<x-ui.sccr-select name="filterTrash" wire:model.live="filterTrash" :options="$this->filterTrashOptions" class="w-32" />
</div>
...
<tr class="hover:bg-gray-200 {{ $item->trashed() ? 'opacity-60' : '' }} transition">
{{-- ROW CHECKBOX --}}
...
{{-- Status --}}
<td class="px-3 py-2 text-center">@php $isTrashed = $item->trashed();
...
@endphp
@if($isTrashed)
<span class="px-2 py-0.5 rounded text-xs bg-red-200 text-red-800 line-through">
{{ $statusLabel }} (Deleted)
</span>
@else
...
@endif
          ...
          @else
            ...
            @if ($canDelete)
            <x-ui.sccr-button type="button" variant="icon"
              wire:click="restorePR('{{ $item['id'] }}')"
              class="text-green-600 hover:scale-125"
              wire:confirm="Are you sure you want to restore PR {{ $item['pr_number'] }}?"
              title="Restore">
              <x-ui.sccr-icon name="rotate-left" :size="18" />
            </x-ui.sccr-button>
            @endif
          @endif
</div>
