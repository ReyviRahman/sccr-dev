<x-ui.sccr-card wire:key="students-table" transparent>
    ...
    <div class="relative px-8 py-1 bg-blue-500/60 rounded-b-3xl shadow-lg overflow-hidden">
        <div class="flex justify-between items-center px-4">
            ...
            <div class="flex items-center space-x-3">
                <x-ui.sccr-button wire:click="openCreateModal" variant="primary">
                    Tambah Data
                </x-sccr-button>

                <x-ui.sccr-button wire:click="openCreateAddModal" variant="primary">
                    Quick Add
                </x-sccr-button>
            </div>
    ...
    <form wire:submit.prevent="applyFilter" class="flex flex-wrap items-center justify-between px-4 py-2 gap-3">
        ...
        <div class="flex flex-wrap items-center gap-3 flex-1">
            ...
            <x-ui.sccr-input name="search" wire:model.blur="search" placeholder="Cari Nama, NIM, Prodi, Fakultas"
                class="w-64" />

            {{-- Filter Fakultas --}}
            <x-ui.sccr-select name="fakultas_id" wire:model.defer="fakultas_id" :options="$faculties" label="Fakultas"
                class="w-40" />

            {{-- Filter Prodi --}}
            <x-ui.sccr-select name="prodi_id" wire:model.defer="prodi_id" :options="$studyPrograms" label="Prodi"
                class="w-40" />

            {{-- Tahun Masuk --}}
            <x-ui.sccr-select name="tahun_masuk" wire:model.defer="tahun_masuk" :options="$entryYearOptions" label="Tahun Masuk"
                class="w-32" />

            {{-- Status --}}
            <x-ui.sccr-select name="student_status" wire:model.defer="student_status" :options="$statusOptions" class="w-32" />

            <x-ui.sccr-button type="submit" variant="primary">Cari</x-sccr-button>
    ...
    {{-- Modal Create --}}
    @if ($showCreateModal)
        <x-ui.sccr-modal :show="$showCreateModal" title="Tambah Data Mahasiswa" maxWidth="2xl">
    ...
    @if ($showCreateAddModal)
        <x-ui.sccr-modal :show="$showCreateAddModal" title="Quick Add Mahasiswa" maxWidth="2xl">
