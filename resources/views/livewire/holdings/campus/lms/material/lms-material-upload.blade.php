<x-ui.sccr-card wire:key="lms-material-upload">
    ...
    <form wire:submit.prevent="upload" class="grid grid-cols-1 gap-4 px-4 py-6 max-w-xl">
        <x-ui.sccr-input name="title" wire:model.defer="title" label="Judul Materi" />
        <x-ui.sccr-input name="description" wire:model.defer="description" label="Deskripsi" type="textarea" />
        <x-ui.sccr-select name="type" wire:model.defer="type" label="Tipe Materi" :options="[
            'document' => 'Dokumen',
            'video' => 'Video',
            'slide' => 'Slide',
            'link' => 'Tautan',
        ]" />
        <x-ui.sccr-input name="file_path" wire:model.defer="file_path" label="Path File / URL" />

        <div class="flex gap-3 mt-4">
            <x-ui.sccr-button type="submit" variant="primary">Upload</x-sccr-button>
