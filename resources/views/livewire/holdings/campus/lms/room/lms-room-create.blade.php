<x-ui.sccr-card wire:key="lms-room-create">
    ...
    <form wire:submit.prevent="save" class="grid grid-cols-1 gap-4 px-4 py-6 max-w-xl">
        <x-ui.sccr-input name="name" wire:model.defer="name" label="Nama Room"
            placeholder="Contoh: Pemrograman Web A" />
        @error('name')
            <span class="text-red-600 text-sm">{{ $message }}</span>
        @enderror

        <x-ui.sccr-input name="kurikulum" wire:model.defer="kurikulum" label="Kurikulum" placeholder="Contoh: RPL 2025" />
        @error('kurikulum')
            <span class="text-red-600 text-sm">{{ $message }}</span>
        @enderror

        <x-ui.sccr-input name="semester" wire:model.defer="semester" label="Semester"
            placeholder="Contoh: Ganjil 2025/2026" />
        @error('semester')
            <span class="text-red-600 text-sm">{{ $message }}</span>
        @enderror

        <x-ui.sccr-input type="number" name="max_participants" wire:model.defer="max_participants" label="Max Peserta"
        ...
        <div class="flex gap-3 mt-4">
            <x-ui.sccr-button type="submit" variant="primary">Buat Room</x-sccr-button>
