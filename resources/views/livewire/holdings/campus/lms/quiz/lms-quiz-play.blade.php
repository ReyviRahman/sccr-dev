<x-ui.sccr-card wire:key="lms-quiz-play">
        ...
        @foreach ($questions as $index => $question)
            <div class="bg-white shadow rounded p-4">
                ...
                @if ($question->type === 'multiple_choice')
                    ...
                    @endforeach
                @else
                    <x-ui.sccr-input type="textarea" wire:model.defer="answers.{{ $question->id }}"
                        placeholder="Jawaban Anda..." />
                @endif
            </div>
        @endforeach

        <x-ui.sccr-button wire:click="submitQuiz" variant="primary">Kirim Jawaban</x-sccr-button>
