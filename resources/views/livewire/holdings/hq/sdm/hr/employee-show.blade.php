<x-ui.sccr-card class="max-w-6xl mx-auto p-4">
    <div class="space-y-2">
        ...
        {{-- Pengaturan Absensi & Meal --}}
        <div class="grid grid-cols-1 gap-4">
            <div class="bg-white p-4 border rounded shadow">
                <h3 class="font-semibold text-sm text-gray-800 mb-3">Pengaturan Absensi & Meal</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="flex text-xs p-1">
                        <span class="w-40 font-semibold">Wajib Absensi</span>
                        <span class="mr-1">:</span>
                        <span>{{ $employee->is_attendance_required ? 'Ya' : 'Tidak' }}</span>
                    </div>
                    <div class="flex text-xs p-1">
                        <span class="w-40 font-semibold">Multi-Lokasi</span>
                        <span class="mr-1">:</span>
                        <span>{{ $employee->is_multi_location_attendance_allowed ? 'Ya' : 'Tidak' }}</span>
                    </div>
                    <div class="flex text-xs p-1">
                        <span class="w-40 font-semibold">Makan Siang</span>
                        <span class="mr-1">:</span>
                        <span>{{ $employee->is_company_lunch_eligible ? 'Ya' : 'Tidak' }}</span>
                    </div>
                    <div class="flex text-xs p-1">
                        <span class="w-40 font-semibold">Snack</span>
                        <span class="mr-1">:</span>
                        <span>{{ $employee->is_company_snack_eligible ? 'Ya' : 'Tidak' }}</span>
                    </div>
                    <div class="flex text-xs p-1">
                        <span class="w-40 font-semibold">Budget Makan/Hari</span>
                        <span class="mr-1">:</span>
                        <span>Rp {{ number_format((float) $employee->daily_lunch_budget, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex text-xs p-1">
                        <span class="w-40 font-semibold">Budget Snack/Hari</span>
                        <span class="mr-1">:</span>
                        <span>Rp {{ number_format((float) $employee->daily_snack_budget, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex text-xs p-1">
                        <span class="w-40 font-semibold">Metode Potong Gaji</span>
                        <span class="mr-1">:</span>
                        <span>{{ $employee->meal_deduction_method ?? '-' }}</span>
                    </div>
                    <div class="flex text-xs p-1">
                        <span class="w-40 font-semibold">Catatan</span>
                        <span class="mr-1">:</span>
                        <span>{{ $employee->attendance_note ?? '-' }}</span>
                    </div>
                </div>
            </div>
        </div>
