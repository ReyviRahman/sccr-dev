...
<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 px-4 py-6">
        ...
        <x-sccr-section title="Room Management">
            <x-ui.sccr-button :href="route('holdings.campus.lms.room.create')">Buat Room</x-sccr-button>
            <x-ui.sccr-button :href="route('holdings.campus.lms.room.join', ['room' => $room->id ?? 0])">Gabung Room</x-sccr-button>
        </x-sccr-section>

        <x-sccr-section title="Quiz">
            <x-ui.sccr-button :href="route('holdings.campus.lms.quiz')">Daftar Kuis</x-sccr-button>
            <x-ui.sccr-button :href="route('holdings.campus.lms.quiz.create', ['room' => $room->id ?? 0])">Buat Kuis</x-sccr-button>
        </x-sccr-section>

        <x-sccr-section title="Material">
            <x-ui.sccr-button :href="route('holdings.campus.lms.material.upload', ['room' => $room->id ?? 0])">Upload Materi</x-sccr-button>
        </x-sccr-section>


        {{-- Dashboard --}}
        {{-- <x-ui.sccr-card title="Dashboard LMS">
            @livewire('dashboard.lms-dashboard')
        </x-sccr-card> --}}

        {{-- Buat Room --}}
        {{-- <x-ui.sccr-card title="Buat Room LMS">
            @livewire('holdings.campus.lms.lms-room-create')
        </x-sccr-card> --}}

        {{-- Kelola Room --}}
        {{-- <x-ui.sccr-card title="Kelola Room LMS">
            @isset($room)
        ...
        {{-- Upload Materi --}}
        {{-- <x-ui.sccr-card title="Upload Materi Belajar">
            @livewire('holdings.campus.lms.lms-material-upload')
        </x-sccr-card> --}}

        {{-- Buat Kuis --}}
        {{-- <x-ui.sccr-card title="Buat Kuis">
            @livewire('holdings.campus.lms.lms-quiz-create')
        </x-sccr-card> --}}

        {{-- Kerjakan Kuis --}}
        {{-- <x-ui.sccr-card title="Kerjakan Kuis">
            @isset($quiz)
        ...
        {{-- Hasil Kuis --}}
        {{-- <x-ui.sccr-card title="Lihat Hasil Kuis">
            @isset($quiz)
        ...
        {{-- Gabung Room --}}
        {{-- <x-ui.sccr-card title="Gabung Room LMS">
            @livewire('holdings.campus.lms.lms-room-join')
        </x-sccr-card> --}}

        {{-- Video Frame --}}
        {{-- <x-ui.sccr-card title="Sesi Video LMS">
