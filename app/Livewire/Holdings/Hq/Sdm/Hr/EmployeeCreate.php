...
class EmployeeCreate extends Component
{
    ...
    public $is_attendance_required = true;

    public $is_multi_location_attendance_allowed = false;

    public $is_company_snack_eligible = true;

    public $is_company_lunch_eligible = true;

    public $daily_lunch_budget = 0;

    public $daily_snack_budget = 0;

    public $meal_deduction_method = 'auto_deduct';
    ...
    protected $rules = [
        ...
        'is_attendance_required' => 'nullable|boolean',
        'is_multi_location_attendance_allowed' => 'nullable|boolean',
        'is_company_snack_eligible' => 'nullable|boolean',
        'is_company_lunch_eligible' => 'nullable|boolean',
        'daily_lunch_budget' => 'nullable|numeric|min:0',
        'daily_snack_budget' => 'nullable|numeric|min:0',
        'meal_deduction_method' => 'nullable|string',
    ];
    ...
    public function save()
    {
        ...
        DB::transaction(function () use (&$employeeNip) {
            $employee = Emp_Employee::create([
                'nip' => $this->nip,
            ...
            $employeeNip = (string) $employee->nip;

            DB::afterCommit(function () use ($employeeNip) {
                if ($employeeNip === '') {
                    return;
                }
                ...
                try {
                    $result = app(EmployeeDeviceProvisionOrchestratorService::class)
                        ->processEmployeeNow($employeeNip, 20);
        ...
        // tetap pertahankan event lama jika ada pemakai lain
        $this->dispatch('employeeCreated');

        // event yang cocok dengan EmployeeTable
        $this->dispatch('emp-employee-created');

        session()->flash('success', 'Karyawan berhasil ditambahkan. Provisioning device diproses.');
