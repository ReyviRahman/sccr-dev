...
class EmployeeEdit extends Component
{
    ...
    public ?bool $is_multi_location_attendance_allowed = false;

    public ?bool $is_company_snack_eligible = true;

    public ?bool $is_company_lunch_eligible = true;

    public ?float $daily_lunch_budget = 0;

    public ?float $daily_snack_budget = 0;

    public ?string $meal_deduction_method = 'auto_deduct';
    ...
    private function loadEmployee(string $nip): void
    {
        ...
        $this->is_attendance_required = $e->is_attendance_required ?? true;
        $this->is_multi_location_attendance_allowed = $e->is_multi_location_attendance_allowed ?? false;
        $this->is_company_snack_eligible = $e->is_company_snack_eligible ?? true;
        $this->is_company_lunch_eligible = $e->is_company_lunch_eligible ?? true;
        $this->daily_lunch_budget = (float) ($e->daily_lunch_budget ?? 0);
        $this->daily_snack_budget = (float) ($e->daily_snack_budget ?? 0);
        $this->meal_deduction_method = $e->meal_deduction_method ?? 'auto_deduct';
    }
    ...
    public function update(): mixed
    {
        ...
        $before = Emp_Employee::query()
            ->where('nip', $this->nip_original)
            ->firstOrFail();

        $oldNip = (string) $before->nip;
        $newNip = (string) $this->nip;

        // hanya field yang relevan ke Hikvision
        $provisionRelevantChanged =
            $oldNip !== $newNip
            || (string) ($before->nama ?? '') !== (string) ($this->nama ?? '')
            || (int) ($before->holding_id ?? 0) !== (int) ($this->holding_id ?? 0)
            || (string) ($before->employee_status ?? '') !== (string) ($this->employee_status ?? '');

        DB::transaction(function () use ($oldNip, $newNip, $provisionRelevantChanged) {
            Emp_Employee::where('nip', $this->nip_original)->update([
                ...
                'is_attendance_required' => $this->is_attendance_required,
                'is_multi_location_attendance_allowed' => $this->is_multi_location_attendance_allowed,
                'is_company_snack_eligible' => $this->is_company_snack_eligible,
                'is_company_lunch_eligible' => $this->is_company_lunch_eligible,
                'daily_lunch_budget' => $this->daily_lunch_budget,
                'daily_snack_budget' => $this->daily_snack_budget,
                'meal_deduction_method' => $this->meal_deduction_method,
            ]);

            if ($provisionRelevantChanged) {
                $targetNips = array_values(array_unique(array_filter([
                    $oldNip,
                    $newNip,
                ])));

                DB::afterCommit(function () use ($targetNips) {
                    foreach ($targetNips as $targetNip) {
                        try {
                            $result = app(EmployeeDeviceProvisionOrchestratorService::class)
                                ->processEmployeeNow((string) $targetNip, 20);

                            Log::info('EMPLOYEE_IMMEDIATE_PROVISION_RESULT', $result);
                        } catch (\Throwable $e) {
                            Log::error('EMPLOYEE_IMMEDIATE_PROVISION_ERROR', [
                                'employee_nip' => (string) $targetNip,
        ...
        session()->flash('success', 'Data karyawan berhasil diperbarui. Provisioning device diproses.');
