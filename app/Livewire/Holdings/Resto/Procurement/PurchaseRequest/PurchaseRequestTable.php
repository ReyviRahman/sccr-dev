...
class PurchaseRequestTable extends Component
{
    ...
    public string $filterTrash = ''; // '', 'active', 'trashed'
    ...
    protected function dataQuery(): Collection
    {
        ...
        if ($this->filterTrash === 'trashed') {
            $query->withoutGlobalScopes(['Illuminate\Database\Eloquent\SoftDeletingScope'])->whereNotNull('deleted_at');
        } elseif ($this->filterTrash === 'active') {
    ...
    public function getFilterTrashOptionsProperty(): array
    {
        return [
            '' => 'All',
            'active' => 'Active',
    ...
    {
        $this->reset('search', 'filterStatus', 'filterLocation', 'filterTrash');
        $this->resetPage();
    }
    ...
    public function restorePR(string $id): void
    {
        try {
            PurchaseRequestService::restorePR((int) $id);
            $this->resetPage();
            $this->toast = ['show' => true, 'type' => 'success', 'message' => 'Purchase Request berhasil dipulihkan.'];
        } catch (\Exception $e) {
            $this->toast = ['show' => true, 'type' => 'error', 'message' => $e->getMessage()];
        }
    }
