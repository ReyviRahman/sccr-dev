...
class PurchaseRequestService
{
    ...
    /**
     * Soft delete PR (data akan masuk ke trash)
     */
    ...
    /**
     * Restore PR from trash
     */
    public static function restorePR(int $prId): Rst_PurchaseRequest
    {
        return DB::transaction(function () use ($prId) {
            $pr = Rst_PurchaseRequest::withTrashed()->findOrFail($prId);

            if (! $pr->trashed()) {
                throw new \Exception('PR tidak dalam status terhapus.');
            }
    ...
    /**
     * Force delete PR permanently
     */
    public static function forceDeletePR(int $prId): void
    {
        DB::transaction(function () use ($prId) {
            $pr = Rst_PurchaseRequest::withTrashed()->findOrFail($prId);
